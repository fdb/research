...
function evalChildUncached(ctx, networkPath, network, child, networkArgs, path) {
  ...
  for (const pub of network.publishedPorts || []) {
    publishedByRef.set(`${pub.child}.${pub.port}`, pub);
  }
  ...
  for (const port of ports) {
    ...
    const pub = publishedByRef.get(`${child.name}.${port.name}`);
    if (pub !== undefined && pub.name in networkArgs) {
      const v = networkArgs[pub.name];
      // Java (NodeContext.renderChild): a published LIST value becomes the
      // child port's whole values list — one invocation per element for
      // value-range ports; everything else arrives as a single value, and
      // published data overrides ports WITHOUT type conversion. Cartan's
      // list-processing nodes (drop_last, contours/redraw…) depend on it.
      portLists.push({ port, list: Array.isArray(v) ? v : [v] });
      continue;
    }
    ...
    if (conn) {
      ...
      // An unconnected list-range port is an EMPTY list, not [null] —
      // otherwise list.combine & friends would emit spurious nulls.
      portLists.push({ port, list: literal == null && port.range === "list" ? [] : [literal] });
    }
  }
  ...
  for (const argumentMap of argumentMaps) {
    ...
    if (type.outputRange === "list") {
      ...
      // Java (NodeContext.postProcessResult): a value-range node with
      // geometry output returning a list of POINTS flattens into the
      // stream; any other returned list stays one list-valued result.
      if (
        Array.isArray(raw) &&
        type.outputType === "shape" &&
        raw.length > 0 &&
        raw.every((v) => isPoint(v))
      ) {
        for (const v of raw) results.push(v);
      } else {
        results.push(raw);
...
/**
 * Cartan-port deviation from the stricter unified engine: null flows
 * through every conversion (Java lets functions decide about null), and
 * values with no defined conversion pass RAW instead of throwing —
 * NodeBox 3 only converts known type pairs and otherwise trusts the
 * function, which Cartan's table-through-geometry-port wiring relies on.
...
export function convertList(list, port, path) {
  switch (port.type) {
    ...
    case "shape":
      // Java (NodeContext.convertResultsForPort): a list of POINTS flowing
      // into a value-range geometry port becomes ONE list-value — the node
      // is invoked once with the whole point list ("work with either
      // single IGeometry objects or a list of Points").
      if (port.range !== "list" && list.length > 0 && list.every((v) => isPoint(v))) {
        return [list];
      }
      return list;
    case "float":
    case "int":
      return list.map((v) => {
        ...
        // NaN is a legal double in Java and flows through NodeBox graphs
        // (Cartan's complex-number tricks depend on it).
        if (typeof n !== "number") {
    ...
    case "string":
      return list.map((v) => (v == null ? v : stringify(v)));
    case "boolean":
      return list.map((v) =>
        v == null ? v : typeof v === "string" ? v.toLowerCase() === "true" : Boolean(v),
      );
    case "color":
      return list.map((v) => {
        if (v == null) return v;
