// nodebox-cartan-library/engine/graphics.js
// Vendored from /nodebox/core/graphics.js (the unified-NodeBox engine)
// and extended for the Cartan library port with:
//   - a 'text' shape type (NodeBox 3's textpath, drawn as native text
//     rather than font outlines — see ../doc/DECISIONS.md),
//   - approximate text metrics (Helvetica advance widths) so bounds()
//     works identically in the browser and in headless Node,
//   - roundedRectPath (corevector.rect's roundness port).
...
export function transformShape(t, shape) {
  ...
  // Java's geometry ports may carry a bare list of points.
  if (Array.isArray(shape)) return shape.map((s) => transformShape(t, s));
  if (!isShape(shape)) return shape; // non-geometry passes through untouched
  if (shape.type === "group") {
    return makeGroup(shape.shapes.map((s) => transformShape(t, s)));
  }
  if (shape.type === "text") {
    return { ...shape, transform: compose(t, shape.transform || IDENTITY) };
  }
...
export function isShape(v) {
  return (
    v != null &&
    typeof v === "object" &&
    (v.type === "path" || v.type === "group" || v.type === "text")
  );
}
...
export function bounds(shape) {
  ...
  const walk = (s) => {
    ...
    else if (isPoint(s)) add(s.x, s.y);
    else if (s.type === "group") s.shapes.forEach(walk);
    else if (s.type === "text") {
      for (const c of textCorners(s)) add(c.x, c.y);
    } else if (s.type === "path") {
...
export function shapePoints(shape) {
  ...
  if (Array.isArray(shape)) return shape.flatMap(shapePoints);
  if (shape.type === "group") return shape.shapes.flatMap(shapePoints);
  if (shape.type === "text") return textCorners(shape); // approximation: bounds corners
  // Anchor points only, tagged with the NodeBox point type (1 LINE_TO /
  // 2 CURVE_TO). Java's point stream also carries CURVE_DATA control
  // points; including them turned degenerate curves into duplicate
  // stream points and broke Cartan's segment math, so this port leaves
  // them out (documented deviation — see doc/DECISIONS.md).
  return shape.commands
    .filter((c) => c.type !== "Z")
    .map((c) => ({ x: c.x, y: c.y, ptype: c.type === "C" ? 2 : 1 }));
}
...
export function drawShape(ctx, shape) {
  ...
  if (shape.type === "text") {
    ...
    if (t) ctx.transform(t.a, t.b, t.c, t.d, t.tx, t.ty);
    ctx.font = `${shape.fontSize}px "${shape.fontName}", "Helvetica Neue", Helvetica, Arial, sans-serif`;
    ctx.textAlign = shape.align === "RIGHT" ? "right" : shape.align === "CENTER" ? "center" : "left";
    ctx.textBaseline = "alphabetic";
    ctx.fillStyle = toCSS(shape.fill || BLACK);
    const anchorX = shape.align === "CENTER" ? shape.x + shape.width / 2 : shape.align === "RIGHT" ? shape.x + shape.width : shape.x;
    ctx.fillText(shape.text, anchorX, shape.y);
    if (shape.stroke && shape.strokeWidth > 0) {
      ctx.strokeStyle = toCSS(shape.stroke);
      ctx.lineWidth = shape.strokeWidth;
      ctx.strokeText(shape.text, anchorX, shape.y);
    }
    ctx.restore();
    return;
  }
...
export function toPathData(path) {
  const f = (n) => (Number.isFinite(n) ? +n.toFixed(3) : 0);
...
function svgFragment(value) {
  ...
  if (value.type === "text") {
    ...
    const tr = t ? ` transform="matrix(${t.a} ${t.b} ${t.c} ${t.d} ${t.tx} ${t.ty})"` : "";
    const anchor =
      value.align === "CENTER" ? "middle" : value.align === "RIGHT" ? "end" : "start";
    const anchorX =
      value.align === "CENTER"
    ...
    return (
      `<text x="${anchorX}" y="${value.y}" text-anchor="${anchor}"` +
      ` font-family="${value.fontName}, Helvetica, Arial, sans-serif" font-size="${value.fontSize}"` +
      ` fill="${value.fill ? toCSS(value.fill) : "black"}"${tr}>${esc(value.text)}</text>`
    );
  }
...
// ---------------------------------------------------------------------------
// Text shapes (Cartan-port extension)
// ---------------------------------------------------------------------------
//
// NodeBox 3's textpath converts text to font outlines through AWT. On the
// web (and in headless Node, where thumbnails are pre-rendered) there is
// no font-outline API, so a text shape stays text: it renders via
// fillText / <text>, and MEASURES via an approximate Helvetica advance-
// width table so layout nodes (align, stack, bound_box…) behave the same
// everywhere. Point-level access degrades to the bounds corners.

// Helvetica advance widths per 1000 units of font size, ASCII 32..126.
// prettier-ignore
...
/** Approximate advance width of a string at the given font size. */
export function textWidth(text, fontSize) {
  let w = 0;
  for (const ch of String(text)) {
    const code = ch.codePointAt(0);
    w += code >= 32 && code <= 126 ? HELVETICA_WIDTHS[code - 32] : 600;
  }
  return (w / 1000) * fontSize;
}

/**
 * A text shape. (x, y) is the LEFT end of the baseline like NodeBox's
 * Text grob; align moves the anchor within [x, x+width].
 * @returns {{type:'text'}}
 */
export function makeText(text, x, y, options = {}) {
  return {
    type: "text",
    text: String(text),
    x,
    ...
    fontName: options.fontName || "Verdana",
    fontSize: options.fontSize === undefined ? 24 : options.fontSize,
    align: options.align || "LEFT",
    fill: options.fill === undefined ? BLACK : options.fill,
...
/** The four corners of a text shape's (transformed) bounding box. */
export function textCorners(s) {
  const w = textWidth(s.text, s.fontSize);
  const anchorX =
    s.align === "CENTER" ? s.x + s.width / 2 - w / 2 : s.align === "RIGHT" ? s.x + s.width - w : s.x;
  const ascent = s.fontSize * 0.72;
  const descent = s.fontSize * 0.19;
  const corners = [
    { x: anchorX, y: s.y - ascent },
    { x: anchorX + w, y: s.y - ascent },
    { x: anchorX + w, y: s.y + descent },
    { x: anchorX, y: s.y + descent },
  ];
  return s.transform ? corners.map((c) => transformPoint(s.transform, c)) : corners;
}

/**
 * Rounded rectangle centered on (cx, cy), corner radii (rx, ry) — the
 * roundness port of NodeBox 3's rect.
 * @returns {Path}
 */
export function roundedRectPath(cx, cy, width, height, rx, ry) {
  const w = width / 2;
  const h = height / 2;
  ...
  return makePath([
    { type: "M", x: L + rx, y: T },
    { type: "L", x: R - rx, y: T },
    { type: "C", x1: R - rx + kx, y1: T, x2: R, y2: T + ry - ky, x: R, y: T + ry },
    { type: "L", x: R, y: B - ry },
    { type: "C", x1: R, y1: B - ry + ky, x2: R - rx + kx, y2: B, x: R - rx, y: B },
    { type: "L", x: L + rx, y: B },
    { type: "C", x1: L + rx - kx, y1: B, x2: L, y2: B - ry + ky, x: L, y: B - ry },
    { type: "L", x: L, y: T + ry },
    { type: "C", x1: L, y1: T + ry - ky, x2: L + rx - kx, y2: T, x: L + rx, y: T },
    { type: "Z" },
  ]);
}
